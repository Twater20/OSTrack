...
class TIMOSTrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, TrackingConfig, aux_loss=False, head_type="CORNER", multi_candidate=False, use_adaptive_timesnet=True):
        """ Initializes the model.
        Parameters:
            transformer: torch module of the transformer architecture.
            aux_loss: True if auxiliary decoding losses (loss at each decoder layer) are to be used.
            use_adaptive_timesnet: Whether to use adaptive TimesNet for variable length sequences
        """
        super().__init__()
        self.backbone = transformer
        self.box_head = box_head
        self.use_adaptive_timesnet = use_adaptive_timesnet
        
        if use_adaptive_timesnet:
            # 创建自适应TimesNet配置
            adaptive_config = AdaptiveTrackingConfig(
                max_seq_len=getattr(TrackingConfig, 'max_seq_len', 200),
                min_seq_len=getattr(TrackingConfig, 'min_seq_len', 10),
                pred_len=TrackingConfig.pred_len,
                d_model=TrackingConfig.d_model,
                d_ff=getattr(TrackingConfig, 'd_ff', TrackingConfig.d_model * 4),
                e_layers=TrackingConfig.e_layers,
                top_k=TrackingConfig.top_k,
                num_kernels=TrackingConfig.num_kernels,
                embed=TrackingConfig.embed,
                freq=TrackingConfig.freq,
                dropout=TrackingConfig.dropout,
                num_prediction_heads=getattr(TrackingConfig, 'num_prediction_heads', 4)
            )
            self.timesnet = AdaptiveTimesNetTracking(configs=adaptive_config)
        else:
    ...
    def forward(self, template: torch.Tensor,
        ...
        # TimesNet处理时序信息
        if self.use_adaptive_timesnet:
            # 使用自适应TimesNet
            temporal_output, timesnet_aux = self.timesnet(
                gt_sequence_anno_backward, 
                actual_seq_lens=actual_seq_lens,
                dataset_name=dataset_name
            )
            temporal_features = timesnet_aux['temporal_features']
            
            # 存储额外信息用于损失计算和分析
            aux_dict.update({
                'timesnet_all_predictions': timesnet_aux['all_predictions'],
                'timesnet_selection_weights': timesnet_aux['selection_weights'],
                'timesnet_confidence_scores': timesnet_aux['confidence_scores'],
                'actual_seq_lens': timesnet_aux['actual_seq_lens']
            })
        else:
            # 使用原始TimesNet
...
def build_timostrack(cfg, training=True):
    ...
    model = TIMOSTrack(
        ...
        use_adaptive_timesnet=cfg.MODEL.get('USE_ADAPTIVE_TIMESNET', True),  # 新增：是否使用自适应TimesNet
    )
